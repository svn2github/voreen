...
unsigned long Polygon3D::getMaxFace() const {
    return maxFace_;
}

unsigned long Polygon3D::getNumFaces() const {
    return static_cast<unsigned long>(faces_.size());
}

tgt::vec3 Polygon3D::getFaceNormal(const unsigned long face) const {
...
list<tgt::vec3> Polygon3D::getFaceVertices(const unsigned long id) {
...
Edge3D* Polygon3D::findEdge(EdgeVertex3D* const v1, EdgeVertex3D* const v2) {
    ...
    for (; it != edges_.end(); ++it ) {
        Edge3D* e = *it;
        if ( (e != 0) && e->isPartOf(v1) && e->isPartOf(v2) )
            return *it;
    }
                                ...
                                const unsigned long numIndices, const unsigned long id, const tgt::vec3& faceNormal)
{
    ...
    //
    for (unsigned long k = 0; k < numIndices; ++k ) {
    ...
    if (prF.second == false) {
        // if found, no insertion was made, so delete created one
        //
        delete face;
        face = *(prF.first);
    }
    ...
    for (unsigned long ii = 0; ii < numIndices; ii += 2 ) {
        // ii = indices index
        //
...
PolygonFace3D* Polygon3D::findFace(const unsigned long id) {
...
bool Polygon3D::containsFace(const unsigned long id) {
...
void Polygon3D::clip(const tgt::vec3 &n, const tgt::vec3 &p) {
    ...
    //
    unsigned long startFace = (maxFace_ + 1);

    // check for all vertices if they are on the plane's front, but not
    // within the plane.
    //
    PolygonFace3D::FaceSet newFaces;
    ...
    while ((itV != vertices_.end())) {
        ...
        if (f > 0.00001f) {
            ...
            // vertex causes its adjacent edges to become degenerated and
    ...
    reduceFaces(newFaces, startFace, clippedFace);
}

void Polygon3D::reduceFaces(PolygonFace3D::FaceSet& newFaces, const unsigned long firstID, PolygonFace3D* clippedFace) {
    if ( (clippedFace == 0) || (newFaces.size() <= 0) )
        return;
    ...
    //
    PolygonFace3D::FaceSet::iterator itF1 = newFaces.begin();
    PolygonFace3D* face1 = 0;
    for (; itF1 != newFaces.end(); ++itF1) {
        face1 = *itF1;
...
void Polygon3D::clear() {
    // Do not change the order of deletion! It is crutial!
    //
    ...
    // Deleting the vertices provokes the deletion of edges already
    // so that this loop should be empty...
    //
    Edge3D::EdgeSet::iterator itE = edges_.begin();
    for (; itE != edges_.end(); ++itE )
        delete *itE;
