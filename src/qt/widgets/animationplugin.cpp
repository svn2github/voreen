...
void AnimationPlugin::createWidgets() {
    ...
    recordAnimation_ = new QPushButton(tr("Record Animation"), recordBox_);
    gridLayout->addWidget(recordAnimation_, 0, 1);
...
void AnimationPlugin::createConnections() {
    ...
    connect(recordAnimation_, SIGNAL(clicked()), this, SLOT(recordAnimation()));
...
void AnimationPlugin::updateKeyframe() {
    ...
    if (currentAnimation_.empty() || (size_t)currentKeyframe_ > currentAnimation_.size() - 1)
        return;
...
void AnimationPlugin::startAnimation(bool record) {
    ...
    if (animationState_ != Paused) {
        ...
        curTime += currentAnimation_.at(currentAnimation_.size()-1).timeToNextFrame_;
        knotValues.push_back(curTime);
        ...
        if (record) {
            ...
            MsgDistr.postMessage(new IVec2Msg("msg.resize", ivec2(spinWidth_->value(), spinHeight_->value())));
...
void AnimationPlugin::recordAnimation() {

    QString s = QFileDialog::getExistingDirectory(
                    this,
                    tr("Choose a directory to save the image sequence"));

    recordPathName_ = s.toStdString();
    if (recordPathName_.empty())
        return;
...
void AnimationPlugin::animationStep() {
    ...
    if (animationState_ == Running || animationState_ == Recording) {
        if (currentAnimationFrame_ >= numAnimationFrames_) {
            ...
            float param = float(currentAnimationFrame_) / (numAnimationFrames_-1);
            ...
            if (animationState_ == Recording) {
                ...
                char fn[1024];
                sprintf(fn, "%s%05d%s", std::string(recordPathName_+"/frame").c_str(), currentAnimationFrame_, ".png");
...
void AnimationPlugin::refreshFrameMenu() {
    ...
    for (size_t i = 0; i < currentAnimation_.size(); ++i) {
        QExtAction* act = new QExtAction(QString(tr("Frame %1, %2 secs")).arg
            (i+1).arg(currentAnimation_[i].timeToNextFrame_), this, i);
...
void AnimationPlugin::setWidgetState() {
    ...
    startAnimation_->setEnabled( currentAnimation_.size() >= 2 && (animationState_ == Stopped || animationState_ == Paused) );
...
void AnimationPlugin::loadAnimation() {
    ...
    QString fn = QFileDialog::getOpenFileName(this, tr("Open voreen animation"), ".", tr("Voreen animations (*.vam)"));
