...
void writeScalarFieldToVolume(ScalarField sf, const std::string& outName) {
    ...
    VolumeFloat* target = new VolumeFloat(dimensions);
    target->clear();
    ...
    VolumeHandle* vh = new VolumeHandle(target, tgt::vec3(1.f), tgt::vec3(0.0f));
...
void writeVecFieldToVolume(VecField vf, const std::string& outName) {
    ...
    Volume3xFloat* target = new Volume3xFloat(dimensions);
    target->clear();
    VolumeUInt8* counter = new VolumeUInt8(dimensions);
    counter->clear();
...
void writeVecAndScalarFieldToVolume(VecField vf, ScalarField sf, const std::string& outName) {
    ...
    //Volume4xFloat* target = new Volume4xFloat(dimensions);
    VolumeFloat* target = new VolumeFloat(dimensions);
    VolumeUInt8* counter = new VolumeUInt8(dimensions);
    target->clear();
    counter->clear();
...
void writeSeries(const std::vector<std::string>& fileNames) {
    ...
    DatVolumeReader dvr;
    VolumeHandleBase* vhb = dvr.read(fileNames.at(0))->first();
    if(!vhb)
        return;
