...
template<>
void test(const long double& actual, const long double& expected, const std::string& failureMessage) {
    std::stringstream s;

    s.setf(XmlSerializationConstants::FLOATFORMAT, std::ios::floatfield);
    s.precision(XmlSerializationConstants::FLOATPRECISION);

    // ATTENTION: Since abs and pow are limited to double precision, we have to do arithmetics.
    long double difference = actual - expected;
    if (difference < 0)
        difference = -difference;

    long double epsilon = 1;
    for (int i = 0; i < XmlSerializationConstants::FLOATPRECISION; ++i)
        epsilon *= 0.1;

    s << failureMessage << "[actual: " << actual << ", expected: " << expected << "]";
    test(difference < epsilon, s.str());
}

template<>
void test(const double& actual, const double& expected, const std::string& failureMessage) {
    test(static_cast<const long double>(actual), static_cast<const long double>(expected), failureMessage);
}

template<>
void test(const float& actual, const float& expected, const std::string& failureMessage) {
    test(static_cast<const double>(actual), static_cast<const double>(expected), failureMessage);
}
...
void testSimpleData() {
    ...
    ds.deserialize("f", f);
    ds.deserialize("d", d);
    ds.deserialize("ld", ld);
    ds.deserialize("uip", uip);
    ...
    test(l, static_cast<signed long>(-5), "l incorrect deserialized");
    test(ul, static_cast<unsigned long>(-6), "ul incorrect deserialized");
    test(f, 1.1f, "f incorrect deserialized");
    test(d, 1.2, "d incorrect deserialized");
    test(ld, static_cast<long double>(1.3), "ld incorrect deserialized");

    test(uip, "uip still null");
    test(ip, "ip still null");

    test(uip == &ui, "uip does not point to ui");
...
/**
 * @c UserDefinedData realizes the @c ISerializable interface to implement
...
/**
 * @c UserDefinedDataContainer realizes the @c ISerializable interface for de- and serialization.
 *
 * @par
 * This class contains @c UserDefinedData object and a pointer to an @c UserDefinedData object.
 * As you can see this class contains data which itself realizes the @c ISerializable
...
/**
 * Tests serialization and deserialization of a @c std::vector.
 */
void testVector() {
    std::vector<int> v;
    v.push_back(1);
...
int main() {
    ...
    // Testing std::vector serialization...
    runTest(testVector, "std::vector serialization");
    ...
    // Testing serialization using IAbstractSerializable...
    runTest(testIAbstractSerializable, "serialization using IAbstractSerializable");
