...
public:
    virtual VolumeHandle* apply(const VolumeHandleBase* volume, tgt::vec2 thresholds) const = 0;
};
...
public:
    virtual VolumeHandle* apply(const VolumeHandleBase* volume, tgt::vec2 thresholds) const;
    //Implement isCompatible using a handy macro:
    IS_COMPATIBLE
};

template<typename T>
VolumeHandle* VolumeOperatorIntensityMaskGeneric<T>::apply(const VolumeHandleBase* vh, tgt::vec2 thresholds) const {
    ...
    return new VolumeHandle(subVolume, vh);
}
