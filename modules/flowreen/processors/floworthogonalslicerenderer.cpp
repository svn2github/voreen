...
void FlowOrthogonalSliceRenderer::process() {
    ...
    if (inport_.hasChanged()) {
        volumeDimensions_ = inport_.getData()->getRepresentation<Volume>()->getDimensions();
    ...
    setupVolumeShader(inport_.getData(), &volumeUnit, &transferUnit, &cam, lightPosition_.get()); // also binds the volume
